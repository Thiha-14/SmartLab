...
import { Plus, Edit2, Trash2, Shield, Search, UsersRound, X, CircleCheck, UserGroupIcon as UsersGroup } from 'lucide-react';
    ...
    <div className="space-y-12 animate-in fade-in duration-1000">
          ...
          <div key={group.id} className="bg-white border border-slate-100 rounded-[3rem] p-10 hover:border-blue-300 transition-all duration-500 shadow-sm relative group">
            <div className="flex items-start justify-between mb-8">
              <div className="space-y-1">
                <h3 className="text-3xl font-black text-slate-900 tracking-tight uppercase">{group.name}</h3>
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-3">
                {group.userIds.map(uid => {
                  ...
                  return (
                    <div key={uid} className="flex items-center space-x-3 bg-slate-50 border border-slate-100 px-4 py-2 rounded-xl group-hover:bg-blue-50 transition-colors">
      ...
      {showAdd && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-8 bg-slate-900/40 backdrop-blur-xl animate-in fade-in duration-500">
          <div className="bg-white border border-slate-100 rounded-[4rem] w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl relative animate-in zoom-in-95 duration-500">
            <div className="p-12 border-b border-slate-50 flex justify-between items-center sticky top-0 bg-white/95 backdrop-blur-2xl z-20">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h3 className="text-4xl font-black text-slate-900 tracking-tighter uppercase">Cluster Setup</h3>
                  <p className="text-[11px] font-black text-blue-600 uppercase tracking-[0.3em] mt-1">Personnel Coordination Protocol</p>
                </div>
              </div>
              <button 
            ...
            <div className="p-12 space-y-12">
              <div className="space-y-4">
                <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">Group Designation / Name</label>
                <input type="text" placeholder="e.g. QUANTUM_DYNAMICS_HUB" className="w-full px-7 py-5 rounded-2xl bg-slate-50 border border-slate-100 text-slate-900 focus:ring-1 focus:ring-blue-600/20 focus:bg-white outline-none transition-all font-bold placeholder:text-slate-300" />
              </div>

              <div className="space-y-6">
                <div className="flex items-center justify-between px-1">
                  <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Personnel Allocation (Select Members)</label>
                  <div className="relative group/search">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300" size={14} />
                    <input type="text" placeholder="Search..." className="pl-9 pr-4 py-2 bg-slate-50 border border-slate-100 rounded-xl text-[10px] font-bold outline-none" />
                  </div>
                </div>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
                  {MOCK_USERS.map(user => (
                    <label key={user.id} className="flex items-center space-x-4 p-5 bg-slate-50 border border-slate-100 rounded-2xl cursor-pointer hover:bg-blue-50 hover:border-blue-200 transition-all group">
                      <div className="relative flex items-center">
                        <input type="checkbox" className="w-6 h-6 rounded-lg border-slate-300 text-blue-600 focus:ring-blue-500 cursor-pointer" />
                      </div>
                      <div className="flex flex-col">
                        <span className="text-sm font-black text-slate-900 uppercase tracking-tight">{user.firstName} {user.lastName}</span>
                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-0.5">ID: {user.studentId}</span>
                      </div>
                    </label>
                  ))}
                </div>
